...
/** Date for notification bodies, which are built outside the component. */
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, CategoryCountPipe, TranslateTitlePipe, TimePickerComponent],
...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Typing, the AM/PM toggle and the dropdown all live in <app-time-picker>.
  // What stays here is only what the form itself needs: which clock the
  // language uses, so the fields above can be filled from and read back into
  // the 24-hour times events are stored as.

  /** True when the current language writes times on a 12-hour clock (9:30 PM). */
  get usesTwelveHourClock(): boolean {
    return isTwelveHourLocale(this.i18n.getLocale());
  }

  private invalidTimeError(which: 'start' | 'end'): string {
    const example = this.usesTwelveHourClock
      ? `9:30 ${meridiemLabelFor(this.i18n.getLocale(), 'AM')}`
      : '09:30';
  ...
  get categoryTree(): CategoryNode[] {
    // Built from saved categories as well as the ones events imply. Using only
    // allCategories meant a category was invisible here until some event
    // already used it, so anything created on the Categories tab could not be
    // picked when adding an event, and a new account saw an empty picker no
    // matter how many categories it had made. The Categories tab has always
    // used this union; the pickers now agree with it.
  ...
  categoryPickerAddNew() {
    ...
    const newPath = this.categoryPickerPath.length
      ...
      : name;
    // Saved as well as selected: the category used to exist only on this
    // unsaved form, so abandoning the event took the new category with it.
    this.rememberCategory(newPath);
  ...
  private async loadEventsFromDb(email: string) {
    ...
    if (cached.length > 0) {
      ...
      // Step 2 — nothing in cache: the calendar stays empty. Nothing is ever
      // written here on the user's behalf; every event on the calendar got
      // there because someone asked for it. Two demo logins used to have a
      // year of events seeded in at this point, which meant the app wrote
      // hundreds of records nobody created.
      const shared = await this.eventsService.listSharedEvents(email);
      if (shared.length > 0) {
